...
1. 本章介绍了三种经典的智能体范式:`ReAct`、`Plan-and-Solve` 和 `Reflection`。请分析:
   ...
   - **ReAct: 思考-行动-观察, 再思考, 再行动, "走一步看一步", 动态规划与纠错能力**
   - **Plan-and-solve: 是先思考后行动, 谋定而后动, 一次性形成完整的计划, 工作流程分为规划阶段和执行阶段。**
   - **Reflection: 先行动后反思, 再优化，事后自我循环纠正**
   - 
...
2. 在4.2节的 `ReAct` 实现中，我们使用了正则表达式来解析大语言模型的输出（如 `Thought` 和 `Action`）。请思考:
   ...
   - **这种方法对输出格式高度依赖，一旦模型输出的格式发生轻微变化（例如多了空格、换行、标点不同或字段顺序改变），正则表达式就可能匹配失败。**
   - 
   - 除了正则表达式，还有哪些更鲁棒的输出解析方案？
   - **JSON 格式、函数调用（Function Calling）以及 XML 标签等。**
   - **其中，JSON 是最常见且通用性较强的方案，可以通过标准解析器直接解析字段；**
   - **Function Calling 则通过预定义 schema 对模型输出进行强约束，是目前工业界更稳定的做法；**
   - **XML 标签方式也比纯文本更规范，但使用较少。**
   - 
...
3. 工具调用是现代智能体的核心能力之一。基于4.2.2节的 `ToolExecutor` 设计，请完成以下扩展实践:
   ...
   - **引入失败计数机制,将错误反馈作为 Observation 返回给模型,超过阈值后终止或降级处理**
   - 思考:如果可调用工具的数量增加到$50$个甚至$100$个，当前的工具描述方式是否还能有效工作？在可调用工具数量随业务需求显著增加时，从工程角度如何优化工具的组织和检索机制？
   - **1. 工具分类2. 工具检索（embedding）3. 两阶段决策4. Function Calling**
...
4. `Plan-and-Solve` 范式将任务分解为"规划"和"执行"两个阶段。请深入分析:
   ...
   - **ReAct 更合适, 首先该场景中智能体需要具备调用各种外部的工具的能力, 其次旅行中会发生各种 X 因素, 例如车票售罄, 天气变化等, 需要根据不同的情况做出动态的决策**
   - 
   - 尝试设计一个"分层规划"系统:先生成高层次的抽象计划，然后针对每个高层步骤再生成详细的子计划。这种设计有什么优势？
   - **这种设计类似于习题 1.3 的回答, 首先生成高层次的抽象计划确保了整个行动方案的正确性, 不会偏离方向太多, 在子计划中可以引入 ReAct 和 Reflection 框架的思想, 进行动态的调整和反思, 让整个规划更加灵活**
...
5. `Reflection` 机制通过"执行-反思-优化"循环来提升输出质量。请思考:
   ...
   - **达到质量阈值 或 改进收敛 或 达到最大次数**
   - 
   - 假设你要搭建一个"学术论文写作助手"，它能够生成初稿并不断优化论文内容。请设计一个多维度的Reflection机制，从段落逻辑性、方法创新性、语言表达、引用规范等多个角度进行反思和改进。
   - **在代码案例的基础上, 优化反思模块的提示词, 可以一次性从段落逻辑性、方法创新性、语言表达、引用规范等多角度进行反思，**
   - **或者拆分成多个反思模块, 依次执行, 具体需要看工程效果，模型能力以及响应时长, 拆分成多个模块的优势在于可以选用不同的模型进行不同角度的反思，确定在于 token 消耗过多，响应时间较长**
...
7. 某电商初创公司现在希望使用"客服智能体"来代替真人客服实现降本增效，它需要具备以下功能:
   ...
   - **ReAct + Plan-and-Solve + Reflection（核心三件套）**
   - **其中 ReAct 负责工具调用，Plan-and-Solve 负责流程控制，Reflection 用于风险控制与决策优化**
   - 
   - 这个系统需要哪些工具？请列出至少3个工具及其功能描述。
   - **电商平台订单查询工具, 物流查询工具, 邮件发送工具, 公司政策查询工具**
   - 
   ...
   - **该系统在实际部署中将面临错误决策、幻觉、安全攻击等风险，需要通过结构化工具调用、置信度机制、人机协同以及安全策略加以缓解，从而构建一个可靠、可扩展的智能客服系统。**
   - **对于错误退款和滥批退款, 高金额时触发规则引擎强制约束进行人工兜底**
   - **对于错误的拒绝, 采用低置信度触发 Reflection, 提供补充材料辅助决策**
   - **对于合规/法律问题, 敏感场景的关键字检测, 强制人工介入**
